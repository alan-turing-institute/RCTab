...
automatically monitor usage and shut down cloud resources when they are
...
to specify the duration for which a budget is valid.

The situation with AWS and GCP is similar, in that they also have cost monitoring features, policies/quotas that can reduce costs and customisable spending alerts.
On AWS, these alerts can trigger a built-in action that will turn off EC2 and RDS instances.
Neither has a built-in mechanism to enforce a strict budget across a wide range of services.
...
\item
  ...
  repository has code for the web server, which is pushed
...
in minutes. Additional instances (\textit{e.g.,} for testing) can also be created
quickly.
...
Users can use the web front end to see subscriptions' spending and budget
...
\subsection{Lifecycle of a subscription}\label{lifecycle-of-a-subscription}
...
and with the help of a generous gift from Microsoft Corporation. Oscar
Giles is the original project author, responsible for the initial
design and implementation of the API and CLI.\@ Iain Stenson added the
function apps and newer features of the API and CLI.\@ Tomas Lazauskas
provided guidance and support for the project, as well as
contributions to the API codebase. Joseph Palmer refined the automated
deployment and the front-end details pages, amongst other work. Pamela
Wochner added summary emails, as well as work on preparations for
making the project open-source. In addition to code review and
contributions, Eseoghene Ben-Iwhiwhu provided feedback on the
documentation. We would like to acknowledge code and documentation
contributions by Markus Hauru, Jim Madge and Federico Nanni.
